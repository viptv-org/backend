...
pub(super) fn optional_bool(value: &Value, key: &str) -> Result<Option<bool>, String> {
    value
        .get(key)
...
pub(super) fn action_kind(action: &str) -> &'static str {
    match action {
...
pub(super) fn required_string(value: &Value, key: &str, max: usize) -> Result<String, String> {
    let s = value
        .get(key)
...
pub(super) fn base_url(raw: &str) -> Result<Url, String> {
...
pub(super) fn endpoint(provider: &Provider, endpoint: &str) -> Result<Url, String> {
...
pub(super) fn stream_id(value: &Value, key: &str) -> Option<String> {
...
pub(super) fn episode_extension(value: Option<&Value>) -> String {
...
pub(super) fn extension(value: Option<&str>) -> String {
    let ext = value
        .unwrap_or("mp4")
        .trim()
        .trim_start_matches('.')
...
pub(super) fn decode_epg(v: Option<&Value>) -> String {
...
pub(super) fn valid_year(v: &Value) -> Option<i64> {
    let s = scalar(v)?;
...
pub(super) fn title_year(name: &str) -> (String, Option<i64>) {
    let name = name.trim();
    for (open, close) in [('(', ')'), ('[', ']')] {
...
pub(super) fn imdb_id(v: Option<&Value>) -> Option<String> {
...
pub(super) fn tmdb_id(v: Option<&Value>) -> Option<String> {
    let s = scalar(v?)?;
...
pub(super) fn canonical_id(s: &str) -> String {
