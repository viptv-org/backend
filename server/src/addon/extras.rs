...
pub(super) const MAX_EXTRA_OPTION: usize = 128;

#[derive(Clone, Debug, PartialEq)]
pub(super) struct CatalogExtra {
    pub(super) name: String,
    pub(super) required: bool,
    pub(super) options: Vec<String>,
    pub(super) options_limit: Option<usize>,
...
pub(super) fn bounded_text(value: &Value, limit: usize) -> Option<String> {
...
pub(super) fn bounded_exact_text(value: &Value, limit: usize) -> Option<String> {
    let value = value.as_str()?.trim();
    (!value.is_empty() && value.chars().count() <= limit).then(|| value.to_owned())
}
pub(super) fn extra_name(value: &Value) -> Option<String> {
...
pub(super) fn catalog_extras(catalog: &Value) -> Vec<CatalogExtra> {
...
#[cfg(test)]
pub(super) fn catalog_extra(catalog: &Value, name: &str) -> bool {
